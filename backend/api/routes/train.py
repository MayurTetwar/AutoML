...
async def train(
    ...
    problem_type_classification: Annotated[
        bool, Form(..., description="True = Classification, False = Regression")
    ],
    intensity: Annotated[
        str, Form(..., description="Training intensity — 'low', 'medium', 'high'")
    ],
    model_name: Annotated[
        str,
        Form(
            ...,
            description=f"Classification: {classification_models} | Regression: {regression_models}",
        ),
    ],
    ...
    if not file.filename.endswith((".csv", ".xlsx", ".xls")):
        raise HTTPException(
            status_code=400, detail="Only .csv or .xlsx files are allowed."
        )
    ...
    if intensity not in intensity_map:
        raise HTTPException(
            status_code=400, detail="intensity must be 'low', 'medium', or 'high'."
        )
...
async def train_auto(
    ...
    intensity: Annotated[
        ...
        Form(
            ...,
            description="Training intensity — 'low', 'medium', 'high'. Auto mode uses longer timeouts than manual.",
        ),
    ],
    user_id: str = Depends(get_current_user),
):
