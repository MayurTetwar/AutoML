    ...
    try:
        for col, info in report.items():
            try:
                ...
                print(f"[auto_drop_columns] ERROR processing column '{col}': {type(e).__name__}: {str(e)}")
        ...
        for col, info in report.items():
            try:
                ...
                if info['type'] == 'BOOLEAN':
                    ...
                    plan = ['extract_year', 'extract_month', 'extract_dayofweek','extract_is_weekend']
                    preprocessing_plan[col] = plan
                    ...
                    if pd.api.types.is_numeric_dtype(df[col]):
                        ...
                        preprocessing_plan[col] = ['impute_mode', 'one_hot_encode']  # treat as categorical

            except Exception as e:
                print(f"[decide_preprocessing] ERROR processing column '{col}': {type(e).__name__}: {str(e)}")
                print(f"  Column info: {info}")
                raise
