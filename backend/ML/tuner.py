...
def tune_classifier(preprocessor, X_train, y_train,
    ...
    def objective(trial):
        model_name, model = _get_classifier_model(trial, weight)
        pipeline = Pipeline(steps=[
            ("preprocessor", preprocessor),
            ("model", model)
        ])
        try:
            # cv=2 during search (fast) — we retrain best on full data anyway
            scores = cross_val_score(
                pipeline, X_tune, y_tune,
                cv=2, scoring="accuracy", n_jobs=1  # n_jobs=1 here!
            )
            return scores.mean()
        except Exception:
    ...
    return best_pipeline, best_model_name, best_params
...
def tune_regressor(preprocessor, X_train, y_train, timeout=600):
    ...
    def objective(trial):
        model_name, model = _get_regressor_model(trial)
        pipeline = Pipeline(steps=[
            ("preprocessor", preprocessor),
            ("model", model)
        ])
